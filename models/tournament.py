...
class Tournament:
    ...
    def _generate_pairs(self):
        ...
        if self.current_round == 0:
            ...
            players = sorted(
                self.players,
                key=lambda x: (
                    self.player_scores[x["national_id"]],
                    x["rank"]
                ),
                reverse=True
            )
        ...
        for i in range(0, len(players), 2):
            ...
            for j in range(i + 1, len(players)):
                player2 = players[j]
                if (
                    player2["national_id"] not in used_players and
                    not self._have_played_together(player1, player2)
                ):
    ...
    def _have_played_together(self, player1, player2):
        ...
        for round_obj in self.rounds:
            for match in round_obj.matches:
                if (
                    (match[0][0]["national_id"] == player1["national_id"] and
    ...
    def get_rankings(self):
        ...
        return sorted(
            self.players,
            key=lambda x: (
                self.player_scores.get(x["national_id"], 0),
                x["rank"]
            ),
            reverse=True
        )
    ...
    def load_tournaments():
        ...
        try:
            with open(Tournament.FILE_PATH, "r", encoding="utf-8") as file:
                data = json.load(file)
            tournaments = list(data.get("_default", {}).values())
            if not tournaments:
